 ...
 */
export class GetMealByNameController implements IController {
  /**
   ...
   */
  constructor(
    private getMealUseCase: IGetMealByNameUseCase,
  ...
  async handle(httpRequest: HttpRequest): Promise<IHttpResponse> {
    ...
    if ( httpRequest.path) {
      ...
      if ( pathStringParams.includes("name") ){
        ...
        response = await this.getMealUseCase.execute(name)
      }
      else {
