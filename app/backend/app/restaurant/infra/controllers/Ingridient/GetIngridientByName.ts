 ...
 */
export class GetIngridientByNameController implements IController {
  /**
   ...
   */
  constructor(
    private getIngridientUseCase: IGetIngridientByNameUseCase,
  ...
  async handle(httpRequest: HttpRequest): Promise<IHttpResponse> {
    ...
    if (httpRequest.path) {
      ...
      if (pathStringParams.includes("name")) {
        const name = (httpRequest.path as { name: string }).name;
        // Execute the get ingridients use case
        response = await this.getIngridientUseCase.execute(name);
      } else {
